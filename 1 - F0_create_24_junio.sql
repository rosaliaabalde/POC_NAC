...
-- MAGIC     StructField("RAMO CONT.", StringType, true),
...
-- MAGIC     StructField("TIPO DOC.", StringType, true),
...
-- MAGIC %scala
-- MAGIC //READING CHANGES IN BATCH QUERIES
-- MAGIC // Cargar la tabla con una muestra con 166 de uno de los archivos input
-- MAGIC //val file_location = "/FileStore/tables/2021_04_recibos_sg"
-- MAGIC //val file_location = "/FileStore/tables/F0_input/"
-- MAGIC //val f0_df = spark.read.option("header", "true").options(Map("delimiter"->"|")).option("startingVersion", 0).option("endingVersion", 10).schema(f00_schema).csv(file_location)
...
-- MAGIC // Cargar la tabla con una muestra con 166 de uno de los archivos input
-- MAGIC val file_location = "/FileStore/tables/xaa"
-- MAGIC //val file_location = "/FileStore/tables/F0_input/"
...
-- MAGIC //.when(col("`F.LIQ.CAR.`") <= col("`F.LIQ.COB.`") && col("`F.LIQ.ANU.`") =!= col("`F.LIQ.COB.`") && col("`F.LIQ.COB.`") === "0001-01", lit("DV"))
-- MAGIC .when(col("`F.LIQ.CAR.`") <= col("`F.LIQ.ANU.`") && col("`F.LIQ.ANU.`") =!= "0001-01" && col("`F.LIQ.COB.`") === "0001-01", lit("DV"))
-- MAGIC //.when(col("`F.LIQ.CAR.`") =!= col("`F.LIQ.ANU.`") && col("`F.LIQ.ANU.`") === "0001-01" && col("`F.LIQ.COB.`") === "0001-01", lit("PT"))
...
-- MAGIC %scala
-- MAGIC val columns = spark.catalog.listColumns("f0Temp").select("name").as[String].collect()
